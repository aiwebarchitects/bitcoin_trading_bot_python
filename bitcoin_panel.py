#!/usr/bin/env python3
"""
Bitcoin Trading Bot Panel - Simple monitoring interface for Bitcoin RSI 1MIN Double Confirm bot
Free Hyperliquid Python Trading Bot 4 Bitcoin + Panel
"""
...
class BitcoinPanel:
    def __init__(self, stdscr, use_testnet: bool = None):
        ...
        # Initialize backtest manager and run optimization if needed
        self._initialize_backtest_system()
        
        # Load configuration from settings.py (now with optimized parameters)
    ...
    def _initialize_backtest_system(self):
        """Initialize backtest system - run optimization if results are older than 24 hours"""
        try:
            print("\n" + "="*60)
            ...
            # This will run optimization if needed (results older than 24 hours)
            optimized_params = manager.initialize(force=False)
            
            print("\n" + "="*60)
            print("BACKTEST INITIALIZATION COMPLETE")
            print("="*60)
            print("\nStarting panel in 2 seconds...")
            
            import time
            time.sleep(2)
            
        except Exception as e:
            print(f"\nWarning: Backtest initialization failed: {e}")
    ...
    def get_recent_candles(self, limit: int = 100) -> Optional[pd.DataFrame]:
        """Get recent candle data for Bitcoin from Binance"""
        try:
    ...
    def draw_header(self):
        ...
        title = " BITCOIN RSI 1MIN DOUBLE CONFIRM BOT "
    ...
    def load_trade_history(self):
        """Load Bitcoin trade history from Hyperliquid"""
        try:
            import requests
            ...
            btc_trades.sort(key=lambda x: x['time'], reverse=True)
            self.trade_history = btc_trades[:20]
    ...
    def draw_history_tab(self, start_y):
        ...
        # Section title
        self.stdscr.attron(curses.A_BOLD | curses.color_pair(3))
        self.safe_addstr(y, 2, "BITCOIN TRADE HISTORY (Last 20 Trades)")
        self.stdscr.attroff(curses.A_BOLD | curses.color_pair(3))
        y += 2
    ...
    def draw_backtest_tab(self, start_y):
        ...
        if self.backtest_results and self.backtest_results.get('success'):
            ...
            # Display parameters used in this backtest
            if y < h - 1:
                self.stdscr.attron(curses.A_BOLD)
                self.safe_addstr(y, 4, "Parameters Used:")
                self.stdscr.attroff(curses.A_BOLD)
            y += 1
            
            if y < h - 1:
                self.safe_addstr(y, 6, f"RSI Period: {self.rsi_period}  |  Oversold: {self.oversold_threshold}  |  Overbought: {self.overbought_threshold}")
            y += 1
            
            if y < h - 1:
                self.safe_addstr(y, 6, f"Take Profit: {self.take_profit_pct*100:.1f}%  |  Stop Loss: {abs(self.stop_loss_pct)*100:.1f}%")
            y += 2
    ...
    def draw_position_section(self, start_y):
        ...
        # Section title
        self.stdscr.attron(curses.A_BOLD | curses.color_pair(3))
        self.safe_addstr(y, 2, "BITCOIN POSITION")
        self.stdscr.attroff(curses.A_BOLD | curses.color_pair(3))
        y += 1
